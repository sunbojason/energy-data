...
def timer_trigger_entsoe_ingestion(myTimer: func.TimerRequest) -> None:
    ...
    Saves two files per day:
      - be_market_data_{date}.csv          → core data  → 'entsoe' table
      - be_extended_market_data_{date}.csv → extended   → 'entsoe_extended' table
    """
    ...
    try:
        ...
        if data_df is None or data_df.empty:
            ...
            if actual_rows < expected_min_rows:
                logging.warning(f"INTEGRITY ALERT: Expected at least {expected_min_rows} rows, but got {actual_rows}.")

            file_name = f"be_market_data_{date_str}.csv"
            blob_client = blob_service_client.get_blob_client(container=container_name, blob=file_name)
            blob_client.upload_blob(data_df.to_csv(index=True).encode('utf-8'), overwrite=True)
            logging.info(f"INGESTION SUCCESS: Saved {file_name} ({actual_rows} rows, {len(data_df.columns)} metrics).")
